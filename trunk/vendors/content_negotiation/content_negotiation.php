...
class ContentNegotiation {
    ...
    public function getQ($member, $header = "accept") {
        ...
        switch($header) {
            case 'accept':
                for($a = 0; $a < sizeof($this->{$header}); $a++) {
                    if(array_intersect($this->{$header}[$a], $member)==$member) {
                        return $this->{$header}[$a]['q'];
                    }
                }
                ...
                $major = "$major[0]/*";
                for($a = 0; $a < sizeof($this->{$header}); $a++) {
                    if($this->{$header}[$a]['phpcontentnegotiationmember']==$major) {
                        return $this->{$header}[$a]['q'];
                    }
                }
                for($a = 0; $a < sizeof($this->{$header}); $a++) {
                    if($this->{$header}[$a]['phpcontentnegotiationmember']=="*/*") {
                        return $this->{$header}[$a]['q'];
                    }
                }
            break;
            ...
            default:
                for($a = 0; $a < sizeof($this->{$header}); $a++) {
                    if($this->{$header}[$a]['phpcontentnegotiationmember']==$member['phpcontentnegotiationmember']) {
                        return $this->{$header}[$a]['q'];
                    }
                }
                if($header == 'language') {
                    for($a = 0; $a < sizeof($this->{$header}); $a++) {
                        if($this->{$header}[$a]['phpcontentnegotiationmember']==substr($member['phpcontentnegotiationmember'],0,2)) {
                            return $this->{$header}[$a]['q'];
                        }
                    }
                }
    ...
    public function compareQ($members, $header = "accept") {
    ...
    public function getPref($family = "html") {
    ...
    public function __construct() {
        ...
        if($members=="*/*" || $members=="image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*" && strpos($_SERVER['HTTP_USER_AGENT'], "MSIE")) {
            //special case for Internet Explorer to give it a sensible Accept string according to its actual capabilities
            $members = "text/html,application/xml,image/jpeg,image/pjpeg,image/x-xbitmap,image/gif,image/png;q=0.9,*/*;q=0.1";
        }   
    ...
    private function splitMember($data, $fillQ = 0) {
    ...
    private function determineHeader($header) {
        switch($header) {
            case "accept":
            case "content-type":
            case "type":
    ...
    private function makeString($var) {
        settype($var, "string");
        return $var;
    }
}